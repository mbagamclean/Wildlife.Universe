...
export default function AdminAnimalsPage() {
  ...
  const load = () => db.posts.listByCategory('animals').then((p) => { setPosts(p); setLoaded(true); });
  useEffect(() => { load(); }, []);
    ...
    <div className="p-5 sm:p-8">
      <div className="mb-6 flex items-start justify-between">
        <div>
          ...
          <h1 className="text-2xl font-black sm:text-3xl" style={{ color: 'var(--adm-text)' }}>Animals</h1>
          <p className="mt-1 text-sm" style={{ color: 'var(--adm-text-muted)' }}>Manage animals category posts.</p>
        </div>
        <button
