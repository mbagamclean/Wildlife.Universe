...
const CATEGORY_SLUG = 'birds';
const BLURB =
  'From sovereign skies to coastal flats — the architects of flight in their many forms.';
...
export default async function BirdsPage({ searchParams }) {
  const sp = await searchParams;
  const page = readPage(sp);
  const cat = categories.find((c) => c.slug === CATEGORY_SLUG);
