...
function EmptyState() {
  return (
    ...
    >
      <MapPin className="h-8 w-8 text-[var(--color-primary)] opacity-40" />
      <p className="text-sm text-[var(--color-fg-soft)]">
        No tourism posts yet — add them via the admin panel.
      </p>
    </div>
  );
}
  ...
  const loadPosts = useCallback(async () => {
    try {
      ...
      const filtered = all.filter((p) => {
        ...
        return lbl === 'tourism' || lbl === 'tourism safaris';
      });
        ...
        >
          <div className="flex flex-col gap-1.5">
            ...
            <p className="mt-1 text-sm text-[var(--color-fg-soft)]">
              Wildlife travel destinations, safaris, and unforgettable journeys
            </p>
          </div>
