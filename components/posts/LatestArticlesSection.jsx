...
export function LatestArticlesSection({ initialPosts = null }) {
  ...
  const loadPosts = useCallback(async () => {
    try {
      ...
      const filtered = all.filter((p) => {
        ...
        return lbl === 'articles' || lbl === 'article';
      });
  ...
  return (
    <section
      ...
      style={{
        background: `
          radial-gradient(ellipse at 80% 20%, rgba(48,160,184,0.06) 0%, transparent 50%),
          radial-gradient(ellipse at 20% 80%, rgba(0,128,0,0.04) 0%, transparent 50%),
          var(--color-bg-deep)
        `,
      }}
    >
      ...
      <div className="pointer-events-none absolute right-0 top-0 h-80 w-80 opacity-[0.06] blur-3xl"
        style={{ background: 'radial-gradient(circle, #30a0b8, transparent)' }} />
        ...
        >
          <div className="flex flex-col gap-1.5">
            ...
            <p className="mt-1 text-sm text-[var(--color-fg-soft)]">
              Authority-level wildlife and ecology pieces — long reads from across the natural world
            </p>
          </div>
