...
import { ArrowRight, Bug, ChevronLeft, ChevronRight } from 'lucide-react';
...
function EmptyState() {
  return (
    ...
    >
      <Bug className="h-8 w-8 text-[var(--color-primary)] opacity-40" />
      <p className="text-sm text-[var(--color-fg-soft)]">
        No insect profiles yet — add them via the admin panel.
      </p>
    </div>
  );
}
...
export function LatestInsectsSection() {
  ...
  const loadInsects = useCallback(async () => {
    try {
      const all    = await db.posts.listByCategory('insects');
  ...
  return (
    <section
      ...
      style={{
        background: `
          radial-gradient(ellipse at 20% 30%, rgba(200,128,32,0.06) 0%, transparent 55%),
    ...
    >
      <div className="pointer-events-none absolute -right-20 top-1/4 h-72 w-72 rounded-full opacity-[0.05] blur-3xl"
        style={{ background: 'radial-gradient(circle, #c88020, transparent)' }} />
        ...
        >
          <div className="flex flex-col gap-1.5">
            ...
            <h2 className="font-display text-3xl font-black leading-none tracking-tight text-[var(--color-fg)] sm:text-4xl">
              Explore Latest Insects
            </h2>
            <p className="mt-1 text-sm text-[var(--color-fg-soft)]">
              Discover invertebrates and insect species shaping ecosystems across the planet
            </p>
          </div>
          <Link
            href="/insects"
