...
/**
 * If a string value LOOKS like a JSON object/array, parse and return
 * it. Some legacy `heroes.poster` rows store the upload-result object
 * as a JSON-encoded *string* in the JSONB column (the value comes
 * back wrapped in quotes with escaped inner quotes) instead of as a
 * native JSONB object. Without this unwrap step those rows always
 * resolved to '' and the placeholder lion silhouette painted instead
 * of the real poster image.
 */
...
export function resolveImageUrl(v) {
  ...
  if (typeof v === 'string') {
    // Stringified JSON payload (legacy DB shape) → parse and recurse.
