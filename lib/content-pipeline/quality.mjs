...
// Kept in lockstep with StructuredSchema.faq.min(5) in generate.mjs — if
// Zod accepts 5 entries the gate must too, otherwise a structurally valid
// 5-entry FAQ silently fails the gate, draft sits orphaned, queue row
// burns a daily-cap slot for nothing published.
...
const REQUIRED_SECTION_KEYWORDS = [
  ...
  'iucn',
  'conclusion',
  // Match the model's actual heading variants: "Frequently Asked Questions",
  // "FAQ", "FAQs", "Common Questions". A narrower regex silently rejects
  // articles whose author H2 differs from the canonical phrasing.
  'frequently asked|\\bfaq\\b|common questions',
];
