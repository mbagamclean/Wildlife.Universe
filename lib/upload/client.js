...
export async function uploadVideoDirect(file, { onProgress, signal } = {}) {
  ...
  // 3a. Register the file in the unified media_library so it appears
  //     in /admin/organization/media. Best-effort — never fails the upload.
  fetch('/api/admin/media/register', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({
      path: sign.path,
      ...
      source: 'upload',
      variants: { sources: [originalSource] },
    }),
    signal,
  }).catch(() => {});
...
/**
 * Pull a remote URL through our transcoding pipeline so the result is the
 * same shape uploadMedia returns for files: optimized AVIF + WebP variants
 * for images, and a two-phase {sources, transcodePromise} for videos where
 * the WebM/poster lands later. Same callers, no other changes.
 */
export async function uploadFromUrl(url, { signal } = {}) {
  ...
  const json = await res.json().catch(() => ({}));
  if (!res.ok || !json.success) {
    throw new Error(json.error || `Failed to fetch from URL (${res.status})`);
  }
  const { result } = json;

  // Image: server has already produced the optimized AVIF + WebP variants.
  if (result?.type !== 'video' || !result?.path) return result;

  // Video: server stored the original; kick off the WebM transcode now.
  const baseResult = result;
  const originalSource = result.sources?.[0];
  const transcodePromise = (async () => {
    try {
      const tRes = await fetch('/api/upload/transcode', {
        method: 'POST',
      ...
      return {
        ...
        transcodeSkipReason: data.reason || null,
        bytes: {
          original: data.originalBytes || baseResult.bytes?.original || 0,
          webm: data.webmBytes || 0,
          poster: data.posterBytes || 0,
        },
      };
