...
const heroes = {
  ...
  async create(payload) {
    ...
    // Schema-heal: drop position/active if migration 007 isn't run yet.
    let { data, error } = await db.from('heroes').insert(item).select().single();
    if (error && /position|active/.test(error.message || '')) {
      delete item.position;
      delete item.active;
      ({ data, error } = await db.from('heroes').insert(item).select().single());
    }
  ...
  async update(id, patch) {
    let { data, error } = await getDb().from('heroes').update(patch).eq('id', id).select().single();
    if (error && /position|active/.test(error.message || '')) {
      const safe = { ...patch };
      delete safe.position;
      delete safe.active;
      ({ data, error } = await getDb().from('heroes').update(safe).eq('id', id).select().single());
    }
    if (error) throw new Error(error.message);
    notify();
    return mapHero(data);
