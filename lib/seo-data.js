...
const CARD_COLUMNS = [
  ...
  // author_id is essential for homepage / category byline rendering.
  // Without it every card falls back to the default author (Matt
  // Mclean) regardless of which persona actually wrote the post.
  'author_id',
].join(', ');
...
/**
 * List published posts assigned to a given author slug.
 *
 * Now that posts.author_id holds the author slug directly (populated by
 * scripts/_one-off/backfill-authors.mjs), the lookup is a single indexed
 * equality query — no more client-side filter on a JSON column.
 *
 * Legacy fallback: rows that pre-date the backfill (no author_id set)
 * are matched on the JSON `author.name` field the way we always have.
 * This keeps any historically-curated post still discoverable.
 */
export async function fetchPostsByAuthor(slug, { limit = 50 } = {}) {
  if (!slug) return [];
  const target = String(slug).toLowerCase();

  // Fast path — indexed equality on author_id.
  const { data: fast } = await client()
    .from('posts')
    .select(CARD_COLUMNS + ', author_id')
    .neq('status', 'draft')
    .eq('author_id', target)
    .order('created_at', { ascending: false })
    .limit(limit);

  if (fast && fast.length) return fast.map(mapPost);

  // Legacy fallback — scan rows whose author_id is null, matching on the
  // JSON author.name field. Capped to a sane page count so a non-author
  // slug doesn't pull the whole catalog.
  const { data: legacy } = await client()
    .from('posts')
    .select(CARD_COLUMNS + ', author, author_id')
    .neq('status', 'draft')
    .is('author_id', null)
    .order('created_at', { ascending: false })
    .limit(500);
  if (!legacy || !legacy.length) return [];
  const matched = legacy.filter((row) => {
...
/**
 * Display name for an author slug. Resolved from the static roster in
 * lib/seo/authors.js — no DB round-trip.
 */
export async function fetchAuthorDisplayName(slug) {
  // Import lazily so this module stays free of UI-side deps at the
  // top of seo-data, which is also pulled into route handlers.
  const { getAuthorBySlug } = await import('@/lib/seo/authors');
