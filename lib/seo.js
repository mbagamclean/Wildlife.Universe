...
import { getAuthorBySlug, getDefaultAuthor } from '@/lib/seo/authors';
...
/**
 * Resolve the full author record for a post.
 *
 * Order of precedence:
 *   1. posts.author_id (slug) → lookup in lib/seo/authors.js
 *   2. legacy posts.author JSON object (if any older row still has one)
 *   3. default author (Matt Mclean) as last resort — never null
 *
 * Always returns a real Author with name/bio/photo, so the Article
 * schema and the byline always get a Person entity that Google can
 * evaluate for E-E-A-T.
 */
function resolveAuthor(post) {
  const bySlug = getAuthorBySlug(post?.author_id || post?.authorId);
  if (bySlug) return bySlug;
  const legacy = post?.author;
  if (legacy && typeof legacy === 'object' && legacy.name) {
    // Wrap a legacy JSON author in the same shape so callers don't branch.
    return {
      ...
      photoUrl: legacy.photoUrl || legacy.photo_url || '',
      twitter: legacy.twitter || null,
...
function pickAuthorName(post) {
  return resolveAuthor(post).name || SITE_NAME;
}
...
export function buildPostMetadata(post) {
  ...
  return {
    ...
    authors: [{ name: author, url: resolveAuthor(post).slug ? absoluteUrl(`/author/${resolveAuthor(post).slug}`) : undefined }],
    alternates: {
      canonical: url,
      languages: buildHreflangMap(url),
    },
...
/**
 * @param {object} post — the post row
 * @param {object} [opts]
 * @param {object} [opts.author] — merged author from
 *   `lib/seo/authors-runtime#fetchAuthorBySlug`. If passed, the
 *   Person entity emitted in the JSON-LD reflects the latest admin
 *   overrides; otherwise we fall back to the static module.
 */
export function buildArticleJsonLd(post, opts = {}) {
  if (!post) return null;
  const url = absoluteUrl(postUrl(post));
  const image = pickCoverImage(post);
  const description = pickDescription(post);
  const authorRecord = opts.author || resolveAuthor(post);
  const author = authorRecord.name;
  const authorUrl = authorRecord.slug ? absoluteUrl(`/author/${authorRecord.slug}`) : undefined;
  ...
  return {
    ...
    // Person entity built from the full author record — Google's
    // quality models read jobTitle, description, knowsAbout, and
    // alumniOf as E-E-A-T signals. image is conditional so we don't
    // emit a broken URL when no headshot has been uploaded yet.
    author: {
      '@type': 'Person',
      name: author,
      ...(authorUrl ? { url: authorUrl } : {}),
      ...(authorRecord.title ? { jobTitle: authorRecord.title } : {}),
      ...(authorRecord.bio ? { description: authorRecord.bio } : {}),
      ...(authorRecord.expertise
        ? { knowsAbout: authorRecord.expertise.split(',').map((s) => s.trim()).filter(Boolean) }
        : {}),
      ...(authorRecord.affiliation
        ? { alumniOf: { '@type': 'Organization', name: authorRecord.affiliation } }
        : {}),
      ...(authorRecord.photoUrl
        ? { image: absoluteUrl(authorRecord.photoUrl) }
        : {}),
      ...(authorRecord.website || authorRecord.twitter
        ? { sameAs: [authorRecord.website, authorRecord.twitter].filter(Boolean) }
        : {}),
    },
    publisher: {
      '@type': 'Organization',
